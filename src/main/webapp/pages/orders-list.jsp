...
<script>
	...
	function deleteById(){
		var id = isCheckbox();
		$.post("${pageContext.request.contextPath}/order/delete",
				{"_method":"delete",
					"id":id},
				function (data) {
	...
	function show(pageNo, pageSize) {
		...
		//发送ajax请求
		$.get("${pageContext.request.contextPath}/order/queryAll/" + pageNo + "/" + pageSize +"/" + id, function (data) {
			//此代码要使用jquery进行拼接
			if (data===""){
				...
				for (var i = 0; i < data.list.length; i++) {
					var oid = i+1;
					var order = data.list[i];//取出消息
					var $tr = $("<tr></tr>");
					var $td1 = $("<td><input name='ids' type='checkbox' value='"+order.id+"' onclick='setCheckBok()'></td>");
					var $td2 = $("<td>"+oid+"</td>");
					var $td3 = $("<td>"+order.orderNum+"</td>");
					$tr.append($td1).append($td2).append($td3);
					for (var k = 0;k<order.products.length;k++){
						var $td4 = $("<td>"+data.list[i].products[k].productName+"</td>");
						var $td5 = $("<td class='text-center'>"+(order.products[k].productPrice*order.peopleCount)+"</td>");
						$tr.append($td4).append($td5);
					}
					var $td6 = $("<td class='text-center'>"+order.orderTime+"</td>");
					var $td7 = $("<td class='text-center'>"+(order.orderStatus>0?'已支付':'未支付')+"</td>");
					var $td8 = $("<td class='text-center'><button type='button' class='btn bg-olive btn-xs' onclick=\"location.href='${pageContext.request.contextPath}/orders/queryById/"+ order.id +"'\">详情</td>");
					$tr.append($td6).append($td7).append($td8);
					$tbody.append($tr);
				}
				/*设置总页数和总记录数*/
							...
							<table id="dataList"
								...
								<c:forEach items="${pageInfo}" var="orders">
										<tr>
											...
											<td>${orders.list.id }</td>
											<td>${orders.list.orderNum }</td>
											<%--<td>${orders.list.products.productName }</td>
											<td>${orders.list.products.productPrice }</td>--%>
											<td>${orders.list.orderTimeStr }</td>
											<td class="text-center">${orders.list.orderStatusStr }</td>

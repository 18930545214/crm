	...
	<script>
		...
		function deleteById(id){
			layer.confirm('你确定要删除吗？', {
				...
				$.post("${pageContext.request.contextPath}/permission/delete",
						{"_method":"delete",
							"id":id},
						function (data) {
		...
		function show(pageNo, pageSize) {
			//发送ajax请求
			$.get("${pageContext.request.contextPath}/permission/queryAll", {"pageNo":pageNo,"pageSize":pageSize},function (data) {
				if (data===null){
					layer.alert('权限不足', {icon: 2});
				}else {
					//此代码要使用jquery进行拼接
					var $tbody = $("#permissionList")
					$tbody.empty();// 清空元素中的内容
					// 循环创建tr,td元素
					for (var i = 0; i < data.list.length; i++) {
						var permission = data.list[i];//取出消息
						var $tr = $("<tr></tr>");
						var $td1 = $("<td><input name='ids' type='checkbox' value='"+permission.id+"' onclick='setCheckBok()'></td>");
						var $td2 = $("<td>"+(i+1)+"</td>");
						var $td3 = $("<td>"+permission.permissionName+"</td>");
						var $td4 = $("<td>"+permission.url+"</td>");
						var $td5 = $("<td class='text-center'><a href=\"${pageContext.request.contextPath}/role/findById.do\" class=\"btn bg-olive btn-xs\">详情</a><button type='button' class='btn bg-olive btn-xs' onclick='deleteById(\""+permission.id+"\")'>删除权限</button></td>");

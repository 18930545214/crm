	...
	<script>
		...
		function deleteById(id){
			layer.confirm('你确定要删除吗？', {
				...
				$.post("${pageContext.request.contextPath}/role/delete",
						{"_method":"delete",
							"id":id},
						function (data) {
		...
		function show(pageNo, pageSize) {
			//发送ajax请求
			$.get("${pageContext.request.contextPath}/role/queryAll", {"pageNo":pageNo,"pageSize":pageSize},function (data) {
				if (data===""){
					...
					for (var i = 0; i < data.list.length; i++) {
						var role = data.list[i];//取出消息
						var $tr = $("<tr></tr>");
						var $td1 = $("<td><input name='ids' type='checkbox' value='"+role.id+"' onclick='setCheckBok()'></td>");
						var $td2 = $("<td>"+(i+1)+"</td>");
						var $td3 = $("<td>"+role.roleName+"</td>");
						var $td4 = $("<td>"+role.roleDesc+"</td>");
						var $td5 = $("<td class='text-center'><a href=\"${pageContext.request.contextPath}/role/show/"+ role.id +"\" class=\"btn bg-olive btn-xs\">详情</a><button type='button' class='btn bg-olive btn-xs' onclick='deleteById(\""+role.id+"\")'>删除角色</button><a href=\"${pageContext.request.contextPath}/pages/role-permission-add.jsp?"+ role.id +"\" class=\"btn bg-olive btn-xs\">添加权限</a></td>");
							...
							<div class="pull-left">
								<div class="form-group form-inline">
									<div class="btn-group">
										...
										<button type="button" class="btn btn-default" title="刷新" onclick="refurbish()">
								...
								<thead>
									<tr>
										<th class="" style="padding-right: 0px"><input
											id="selall" type="checkbox" class="icheckbox_square-blue" onclick="checkbox()">
										</th>
								...
								<tbody id="roleList">
										...
										<%--<tr>
											...
											<td class="text-center">
												<a href="${pageContext.request.contextPath}/role/findById.do?id=${role.id}" class="btn bg-olive btn-xs">详情</a><a href="${pageContext.request.contextPath}/role/deleteRole.do?id=${role.id}" class="btn bg-olive btn-xs">删除角色</a><a href="${pageContext.request.contextPath}/role/findRoleByIdAndAllPermission.do?id=${role.id}" class="btn bg-olive btn-xs">添加权限</a>
											</td>
										</tr>--%>
